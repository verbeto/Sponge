...
import org.spongepowered.mod.service.scheduler.ScheduledTask;

import java.util.HashMap;

public class SpongeTimings {
    ...
    public static final CustomTimingsHandler schedulerSyncTimer = new CustomTimingsHandler("** Scheduler - Sync Tasks",
            SpongePluginContainer.pluginParentTimer);
     ...
     * @return
     */
    public static CustomTimingsHandler getPluginTaskTimings(Task task, long period) {
        String plugin;
        final ScheduledTask ctask = (ScheduledTask) task;

        if (task.getOwner() != null) {
            plugin = task.getOwner().getId();
        } else if (!ctask.getName().isPresent()) {
            plugin = "SpongeScheduler";
        } else {
            plugin = "Unknown";
        }
        String taskname = ctask.getName().get();
        ...
        if (result == null) {
            result = new CustomTimingsHandler(name, SpongeTimings.schedulerSyncTimer);
    ...
    /**
     * Get a named timer for the specified tile entity type to track type
     * specific timings.
     * 
     * @param entity
     * @return
